...
function reducer(state, action) {
    switch(action.type) {
        ...
        case ORDER_SET_PAYMENT_TYPE :
            return {...state, order: { ...state.order, paymentType: action.payload }};
        case ORDER_ADD_ITEM : {
            ...
            const itemCounts = orderItems.reduce((a, c) => a + c.quantity, 0);
            const itemPrice = orderItems.reduce((a, c) => a + c.quantity * c.price, 0);
            const taxPrice = Math.round(0.15 * itemPrice * 100) / 100;
            const totalPrice = Math.round((itemPrice + taxPrice) * 100) / 100;
            return {...state, order: {...state.order, orderItems, itemCounts, itemPrice, taxPrice, totalPrice}};
        }
        ...
        case ORDER_CLEAR :
            return {...state, order: {orderItems: [], taxPrice: 0, totalPrice: 0, itemCounts: 0}};
        ...
        case ORDER_LIST_SUCCESS :
            return {...state, orderList: {loading: false, orders: action.payload}};
        ...
        case ORDER_QUEUE_LIST_SUCCESS :
            return {...state, queueList: {loading: false, queue: action.payload}};
