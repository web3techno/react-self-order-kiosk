...
app.get('/api/product/seed', async(req, res) => {
...
const Order = mongoose.model('orders', new mongoose.Schema({
    ...
    inProcess: {type: Boolean, default: true},
...
app.get('/api/orders/queue', async(req, res) => {
    const inProgressOrders = await Order.find({inProcess: true, isCanceled: false}, 'number');
    const servingOrders = await Order.find({isReady: true, isCanceled: false}, 'number');
